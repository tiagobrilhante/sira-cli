<template>
  ...
  <v-row v-if="loadingData">
    <v-col>
      <v-alert
        v-if="dadosEstatisticos.coordenador && dadosEstatisticos.coordenador.nome"
        elevation="15">
        ...
        <v-row>
          <v-col>
            ...
            <v-alert
              elevation="10"
              rounded="xxl">
              <!-- nome e matricula do coordenador -->
              <v-row
                v-if="dadosEstatisticos.coordenador.nome"
                dense>
                <v-col><b>Nome:</b> {{ dadosEstatisticos.coordenador.nome }}</v-col>
                <v-col><b>Matricula:</b> {{ dadosEstatisticos.coordenador.matricula }}</v-col>
              </v-row>

              <!-- cursos coordenados -->
              <v-row dense>
                <v-col>
                  <b>Cursos coordenados:</b> <span v-if="dadosEstatisticos.coordenador.tipo === 'Administrador Geral'"> TODOS</span>
                  <ul v-else>
                    <li
                      v-for="curso in dadosEstatisticos.coordenador.cursos"
                      :key="curso.id">
            ...
            <v-alert
              ...
              <v-btn
                ...
                rounded
                small
                @click="getEstatisticasCoordenadorPorPeriodo('Todos')">Todos
              </v-btn>
              ...
              <v-btn
                ...
                rounded
                small
                @click="getEstatisticasCoordenadorPorPeriodo(periodo)">{{ periodo }}
              </v-btn>
            </v-alert>
            ...
            <v-alert
              ...
              elevation="10"
              rounded="xxl">
              Esse coordenador não realizou nenhum atendimento.
...
import EstatisticaCoordenadorIndividual from './EstatisticaCoordenadorIndividual.vue'
import EstatisticaCoordenadorTodos from './EstatisticaCoordenadorTodos.vue'

export default {
  name: 'EstatisticaCoordenador',
  components: {EstatisticaCoordenadorTodos, EstatisticaCoordenadorIndividual},
  mixins: [logoutMixin],
  props: {
    selectedCoordenador: {
      type: Object,
  ...
  async mounted () {
    await this.getEstatisticasCoordenador()
  },

  methods: {
    async getEstatisticasCoordenador () {
      try {
        const response = await this.$http.get('estatisticas/coordenadorgeral/' + this.selectedCoordenador.id)
    ...
    async getEstatisticasCoordenadorPorPeriodo (periodo) {
      this.selectedPeriodo = periodo
      this.dadosPorPeriodo = {}
      let objetoParaEnvio = {
        coordenadorId: this.selectedCoordenador.id,
        periodo: periodo
      }
      try {
        const response = await this.$http.post('estatisticas/coordenadorperiodo/', objetoParaEnvio)
